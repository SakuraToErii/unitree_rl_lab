...
class PreTrainedPolicyAction(ActionTerm):
    ...
    def __init__(self, cfg: PreTrainedPolicyActionCfg, env: ManagerBasedRLEnv) -> None:
        ...
        # 1. 加载低层策略，read_file 可以读取 Nucleus Server 或本地文件
        policy_file = read_file(self.cfg.policy_path)
        self.low_level_policy = torch.jit.load(policy_file).to(self.device).eval()
        ...
        self.cfg.low_level_observations.velocity_commands.func = lambda env: self._processed_actions
        self.cfg.low_level_observations.velocity_commands.params = dict()

        # 3. 将 last_action 绑定到上一次低层关节动作
        #    last_low_level_action 是零参闭包，包成 lambda env: ... 以匹配 func(env, **params) 调用约定。
        self.cfg.low_level_observations.last_action.func = lambda env: last_low_level_action()
        self.cfg.low_level_observations.last_action.params = dict()
        # 兼容名为 actions 的旧 observation 配置
        if hasattr(self.cfg.low_level_observations, "actions"):
            self.cfg.low_level_observations.actions.func = lambda env: last_low_level_action()
            self.cfg.low_level_observations.actions.params = dict()


        # 4. 创建只含 ll_policy 组的 ObservationManager
        self._low_level_obs_manager = ObservationManager(
            {"ll_policy": self.cfg.low_level_observations}, env
        )
    ...
    def apply_actions(self):
        ...
        if self._counter % self.cfg.low_level_decimation == 0:
            ...
            with torch.inference_mode():
                joint_actions = self.low_level_policy(obs)
            self.low_level_actions[:] = joint_actions
            self._low_level_action_term.process_actions(joint_actions)
